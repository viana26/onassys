  ...
  const handlePrint = () => {
    ...
    const rowsHtml = sortedMov.map((m, idx) => {
      ...
      return `<tr>
        ...
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;text-align:right;font-family:monospace;font-weight:700;color:${natureza === 'entrada' ? '#059669' : '#dc2626'};background:${bg}">${natureza === 'entrada' ? '+' : '-'}${m.quantidade}</td>
        ...
        <div className="p-4 bg-amber-50/20 dark:bg-amber-950/10 border-b border-amber-100 dark:border-[#2d1e0d]">
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[120px]">
              ...
              <SelectSearch
                value={String(filtroTipo)}
                onChange={v => setFiltroTipo(Number(v))}
                options={[{ value: '0', label: 'Todos' }, ...store.tiposMovimentacao.filter(t => t.entidade === 'produto' || t.entidade === 'ambos').map(t => ({ value: String(t.id), label: t.nome }))]}
