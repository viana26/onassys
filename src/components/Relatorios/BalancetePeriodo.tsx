...
export default function BalancetePeriodo({ store, isOpen, onClose }: BalancetePeriodoProps) {
  ...
  }, [store.lancamentos, dataInicio, dataFim, filtroTipo, filtroCategoria, filtroPagamento]);

  const { sortedItems: sortedLancamentos, requestSort, sortConfig } = useSortableData(filteredLancamentos, 'data_lancamento');
  ...
  const ticketMedio = filteredLancamentos.length > 0 ? (totalReceitas + totalDespesas) / filteredLancamentos.length : 0;
  ...
  const exportCSV = () => {
    ...
    const rows = filteredLancamentos.map(l => [
      ...
      l.descricao || '—',
      l.forma_pagamento || '—',
      l.tipo === 'receita' ? l.valor.toFixed(2) : `-${l.valor.toFixed(2)}`,
    ].join(','));
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = filteredLancamentos.map((l, idx) => {
      const bg = idx % 2 === 0 ? '#ffffff' : '#fafaf9';
      const tipoColor = l.tipo === 'receita' ? '#059669' : '#dc2626';
      const tipoLabel = l.tipo === 'receita' ? 'Receita' : 'Despesa';
      const valorFormatado = l.tipo === 'receita' ? formatCurrency(l.valor) : `-${formatCurrency(l.valor)}`;
      return `<tr>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;font-family:monospace;color:#57534e;background:${bg}">${new Date(l.data_lancamento).toLocaleDateString('pt-BR')}</td>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;background:${bg}"><span style="color:${tipoColor};font-weight:600;font-size:9px;text-transform:uppercase">${tipoLabel}</span></td>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;color:#57534e;background:${bg}">${store.categoriaFinanceiroNome(l.categoria_id)}</td>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;font-weight:600;color:#1c1917;background:${bg}">${l.descricao || '—'}</td>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;color:#57534e;background:${bg}">${l.forma_pagamento || '—'}</td>
        <td style="border-bottom:1px solid #e7e5e4;padding:0.5rem 0.75rem;text-align:right;font-family:monospace;font-weight:600;color:${tipoColor};background:${bg}">${valorFormatado}</td>
      </tr>`;
    }).join('');

    printWindow.document.write(`<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Balancete do Período - ${appName()}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  <table style="width:100%;border:none;margin-bottom:1rem"><tr>
    ...
    <td style="width:40%;text-align:right;vertical-align:top;border:none">
      <h2 style="font-size:14px;font-weight:600;color:#1c1917;margin:0">Balancete do Período</h2>
  ...
  <table style="width:100%;border:none;margin-bottom:1rem;font-size:9px;color:#57534e"><tr>
    ...
    <td style="border:none"><strong>Tipo:</strong> ${filtroTipo === 'todos' ? 'Todos' : filtroTipo === 'receita' ? 'Receitas' : 'Despesas'}</td>
    <td style="border:none;text-align:right"><strong>Total de registros:</strong> ${filteredLancamentos.length}</td>
  </tr></table>
  ...
  <table style="width:100%;margin-bottom:1.5rem;border-collapse:collapse;font-size:9px"><tr>
    <td style="width:33%;border:1px solid #d6d3d1;padding:0.75rem;text-align:center;background:#ecfdf5">
      <p style="margin:0;font-size:8px;font-weight:700;color:#059669;text-transform:uppercase;letter-spacing:1px">Receitas</p>
      <p style="margin:6px 0 0 0;font-size:18px;font-weight:700;color:#059669">${formatCurrency(totalReceitas)}</p>
    </td>
    <td style="width:33%;border:1px solid #d6d3d1;padding:0.75rem;text-align:center;background:#fef2f2">
      <p style="margin:0;font-size:8px;font-weight:700;color:#dc2626;text-transform:uppercase;letter-spacing:1px">Despesas</p>
      <p style="margin:6px 0 0 0;font-size:18px;font-weight:700;color:#dc2626">${formatCurrency(totalDespesas)}</p>
    </td>
    <td style="width:33%;border:1px solid #d6d3d1;padding:0.75rem;text-align:center;background:${saldo >= 0 ? '#ecfdf5' : '#fef2f2'}">
      <p style="margin:0;font-size:8px;font-weight:700;color:${saldo >= 0 ? '#059669' : '#dc2626'};text-transform:uppercase;letter-spacing:1px">Saldo</p>
      <p style="margin:6px 0 0 0;font-size:18px;font-weight:700;color:${saldo >= 0 ? '#059669' : '#dc2626'}">${formatCurrency(saldo)}</p>
    </td>
  </tr></table>
  ...
  <table>
    <thead><tr>
      ...
      <th style="border-bottom:2px solid #d97706;padding:0.5rem 0.75rem;text-align:left;font-weight:700;color:#1c1917;background:#f5f5f4;font-size:8px;text-transform:uppercase">Pagamento</th>
      <th style="border-bottom:2px solid #d97706;padding:0.5rem 0.75rem;text-align:right;font-weight:700;color:#1c1917;background:#f5f5f4;font-size:8px;text-transform:uppercase">Valor</th>
    </tr></thead>
    <tbody>${rowsHtml}</tbody>
    <tfoot><tr>
      <td colspan="5" style="border-top:2px solid #d97706;padding:0.625rem 0.75rem;text-align:right;font-weight:700;color:#1c1917;font-size:10px">SALDO:</td>
      <td style="border-top:2px solid #d97706;padding:0.625rem 0.75rem;text-align:right;font-family:monospace;font-weight:700;color:${saldo >= 0 ? '#059669' : '#dc2626'};font-size:10px">${formatCurrency(saldo)}</td>
    </tr></tfoot>
  </table>
  ...
  <div style="margin-top:2.5rem;padding-top:1rem;border-top:1px solid #d6d3d1;font-size:7px;color:#a8a29e;text-align:center">
    ...
    <p style="margin:2px 0 0 0">Balancete do Período | Período: ${periodoLabel}</p>
  </div>
</body></html>`);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 text-xs font-sans">
      <div className="bg-white dark:bg-[#120c06] w-full sm:max-w-5xl max-h-[90vh] rounded-t-2xl sm:rounded-2xl shadow-xl flex flex-col animate-in slide-in-from-bottom border-t border-amber-100 dark:border-[#2d1e0d]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-amber-100 dark:border-[#2d1e0d]">
          <div>
            <h2 className="font-display font-semibold text-lg text-amber-950 dark:text-amber-100">Balancete do Período</h2>
            <p className="text-[10px] text-gray-500 dark:text-amber-100/40 mt-0.5">Resumo financeiro com receitas, despesas e saldo.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className={`p-3 rounded-xl border ${saldo >= 0 ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800'}`}>
              <div className="flex items-center gap-2 mb-1">
                <DollarSign size={14} className={saldo >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'} />
                <span className={`text-[10px] font-bold uppercase ${saldo >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>Saldo</span>
              </div>
              <p className={`text-lg font-bold font-mono ${saldo >= 0 ? 'text-emerald-800 dark:text-emerald-300' : 'text-red-800 dark:text-red-300'}`}>{formatCurrency(saldo)}</p>
            </div>
            <div className="p-3 bg-amber-50 dark:bg-amber-950/20 rounded-xl border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-lg font-bold font-mono text-amber-800 dark:text-amber-300">{filteredLancamentos.length}</p>
            </div>
          </div>
              ...
              <table className="w-full text-left text-xs border-collapse min-w-[600px]">
                <thead>
                  <tr className="bg-amber-50/40 dark:bg-amber-950/20 text-amber-900 dark:text-amber-100 border-b border-amber-100 dark:border-[#22160b]">
                    ...
                    <th className="p-3 whitespace-nowrap"><SortButton label="Pagamento" sortKey="forma_pagamento" sortConfig={sortConfig} onSort={requestSort} /></th>
                    <th className="p-3 text-right pr-4 whitespace-nowrap"><SortButton label="Valor" sortKey="valor" sortConfig={sortConfig} onSort={requestSort} align="right" /></th>
                  </tr>
                </thead>
                <tbody>
                  {sortedLancamentos.map((l, idx) => (
                    <tr key={l.id} className="border-b border-amber-50/50 dark:border-[#22160b]/40 hover:bg-amber-50/20 dark:hover:bg-amber-950/10 transition">
                      ...
                      <td className={`p-3 text-right pr-4 font-mono font-bold whitespace-nowrap ${
                        l.tipo === 'receita' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'
                      }`}>
